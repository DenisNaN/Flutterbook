...
class ContactsEntry extends StatelessWidget {
  ...
  ContactsEntry() {
    ...
    _birthdayEditingController.addListener(() {
      contactsModel.entityBeingEdited.birthday = _birthdayEditingController.text;
    });
  }
    ...
    return ScopedModel(
        ...
        child: ScopedModelDescendant<ContactsModel>(builder:
          ...
          if(avatarFile.existsSync() == false){
            if(inModel.entityBeingEdited != null && inModel.entityBeingEdited.id
            != null){
            ...
            GestureDetector(child: Text("Select from Gallery"),
                onTap: () async{
                  ...
                  if(gallaryImage != null){
                    // Copy the file into the app's docs directory.
                    gallaryImage.saveTo(join(utils.docsDir!.path, "avatar"));
                    // Tell the entry screen to rebuild itself to show the avatar.
                    contactsModel.trrigerRebuild();
                  }
                  // Hide this dialog.
